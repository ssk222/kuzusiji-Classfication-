...
# ★★★ 評価したい画像ファイル名 ★★★
TARGET_FILENAME = "200010454_00016_1.jpg" 

# フォルダ設定
IMAGE_DIR = "./data/10454"
...
class YOLOXWrapper:
    def __init__(self):
        ...
        self.predictor = Predictor(self.model, self.exp, COCO_CLASSES, None, None, "gpu" if DEVICE.type == "cuda" else "cpu", True if DEVICE.type == "cuda" else False, False)
...
class BasePredictor:
    def _load_weights(self, path):
        ...
        except Exception as e: print(f"Error loading {path}: {e}"); sys.exit()
    def _get_transform(self, size):
        return transforms.Compose([SmartPadResize(size), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    def predict(self, img_pil):
        with torch.no_grad():
            output = self.model(self.transform(img_pil).unsqueeze(0).to(DEVICE))
            probs = torch.nn.functional.softmax(output[0], dim=0)
        val, idx = torch.topk(probs, 1)
        return unicode_to_char(self.class_map.get(idx[0].item(), "?")), val[0].item() # 確率も返す

class VGGPredictor(BasePredictor):
    def __init__(self, model_path, class_map, input_size):
        self.class_map = class_map
        self.model = vgg16(weights=None)
        self.model.classifier = nn.Sequential(nn.Linear(512*7*7, 1024), nn.ReLU(True), nn.Dropout(0.5), nn.Linear(1024, 1024), nn.ReLU(True), nn.Dropout(0.5), nn.Linear(1024, len(self.class_map)))
...
class EfficientNetPredictor(BasePredictor):
    def __init__(self, model_path, class_map, input_size, force_classes):
        ...
        self.model.classifier[1] = nn.Linear(self.model.classifier[1].in_features, force_classes)
        self._load_weights(model_path)
        self.transform = self._get_transform(input_size)

class ConvNeXtPredictor(BasePredictor):
    def __init__(self, model_path, class_map, model_name, input_size, force_classes):
        self.class_map = class_map
        self.model = timm.create_model(model_name, pretrained=False, num_classes=force_classes)
        self._load_weights(model_path)
        self.transform = self._get_transform(input_size)
...
if __name__ == "__main__":
    ...
    # 画像検索
    target_path = None
    possible_paths = glob.glob(os.path.join(IMAGE_DIR, "*"))
    for p in possible_paths:
        if os.path.basename(p) == TARGET_FILENAME:
            target_path = p; break
    if target_path is None:
        print(f"Error: Specified image '{TARGET_FILENAME}' not found in {IMAGE_DIR}"); sys.exit()
    ...
    model_conv = ConvNeXtPredictor(CONV_PATH, class_map, CONV_NAME, CONV_SIZE, FORCE_CLASSES_CONV)
    ...
    if os.path.exists(COORDINATE_CSV):
        with open(COORDINATE_CSV, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    if (row[COL_IMAGE] not in name_without_ext) and (name_without_ext not in row[COL_IMAGE]): continue
                    x, y, w, h = int(row[COL_X]), int(row[COL_Y]), int(row[COL_W]), int(row[COL_H])
                    if w<=0 or h<=0: continue
                    gt_boxes.append({'box': [x, y, x+w, y+h], 'char': unicode_to_char(row[COL_UNICODE].strip())})
                except: continue
    ...
    # スコア計算
    stats = {k: {'correct': 0, 'precision': 0, 'recall': 0, 'f1': 0} for k in ['vgg', 'eff', 'conv']}
    ...
    for model_key in ['vgg', 'eff', 'conv']:
        ...
        p = (correct / total_preds * 100) if total_preds > 0 else 0
        f1 = 2 * (p * r) / (p + r) if (p + r) > 0 else 0
        stats[model_key] = {'correct': correct, 'recall': r, 'precision': p, 'f1': f1}

    # 結果表示 (表形式)
    print("\n" + f" COMPARISON RESULT: {TARGET_FILENAME}")
    print("="*60)
    print(f" GT Count: {total_gt} | Pred Count: {total_preds}")
    print("-" * 60)
    print(f" Model           | Correct | Recall | Precision | F1 Score")
    print("-" * 60)
    print(f" VGG16           | {stats['vgg']['correct']:<7} | {stats['vgg']['recall']:.2f}%  | {stats['vgg']['precision']:.2f}%     | {stats['vgg']['f1']:.2f}%")
    print(f" EfficientNetV2  | {stats['eff']['correct']:<7} | {stats['eff']['recall']:.2f}%  | {stats['eff']['precision']:.2f}%     | {stats['eff']['f1']:.2f}%")
    print(f" ConvNeXtV2      | {stats['conv']['correct']:<7} | {stats['conv']['recall']:.2f}%  | {stats['conv']['precision']:.2f}%     | {stats['conv']['f1']:.2f}%")
    print("="*60)
    ...
    # ユーザー指定の描画ロジック関数
    def draw_user_logic(img, model_key):
        ...
        for i, pred in enumerate(predictions):
            ...
            if matched_gt_char:
                ...
                text_pred = f"{pred_char}"
                text_gt   = f"{matched_gt_char}"

                if is_correct:
                    box_color = "blue"; pred_color = "blue"; gt_color = "blue"
                else:
                    box_color = "red"; pred_color = "red"; gt_color = "green"

                draw.rectangle([x1, y1, x2, y2], outline=box_color, width=2)
                bbox = draw.textbbox((0, 0), text_pred, font=font)
                th, tw = bbox[3] - bbox[1], bbox[2] - bbox[0]
                draw.text((x1, y1 - th - 5), text_pred, fill=pred_color, font=font)
                if not is_correct:
                    draw.text((x1 + tw + 10, y1 - th - 5), text_gt, fill=gt_color, font=font)

            else:
                unmatched_color = "magenta"
                draw.rectangle([x1, y1, x2, y2], outline=unmatched_color, width=2)
                display_text = f"Pred:{pred_char} {pred_prob*100:.0f}%"
                draw.text((x1, y1 - 20), display_text, fill=unmatched_color, font=font)
        return img

    # 各モデルの画像生成
    img_vgg = draw_user_logic(img_origin.copy(), 'vgg')
    img_eff = draw_user_logic(img_origin.copy(), 'eff')
    img_conv = draw_user_logic(img_origin.copy(), 'conv')

    # All Compareはシンプル表示 (3つ並べる)
    ...
    for pred in predictions:
        x1, y1, x2, y2 = pred['box']
        label_all = f"V:{pred['vgg']['char']} E:{pred['eff']['char']} C:{pred['conv']['char']}"
        draw_all.rectangle([x1, y1, x2, y2], outline="cyan", width=2)
        bbox = draw_all.textbbox((x1, y1-25), label_all, font=font)
        draw_all.rectangle(bbox, fill="black")
        draw_all.text((x1, y1-25), label_all, fill="white", font=font)
