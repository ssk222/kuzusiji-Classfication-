...
CONV_PATH          = "../vgg/output/model_subcenter/best_model.pth"
CONV_SIZE          = 384
...
OUTPUT_BASE_FOLDER = "./output/conv_subcenter"
...
class SubCenterArcFaceHead(nn.Module):
    ...
    def __init__(self, in_features, num_classes, s=16.0, m=0.3, k=3):
        ...
        self.m    = m
        self.k    = k
        # weight: [num_classes * k, in_features]
    ...
    def forward(self, features, labels=None):
        ...
        # 全サブセンターとのcosine: [B, num_classes * k]
        cosine_all = F.linear(F.normalize(features), F.normalize(weight))

        # クラスごとにK個のサブセンターから最大値を取る: [B, num_classes]
...
class ConvNeXtSubCenterPredictor(BasePredictor):
    """Sub-center ArcFace付きConvNeXtの推論クラス（Top5対応）"""
...
def draw_result(img, predictions, gt_boxes, font):
    ...
    for pred in predictions:
        ...
        if matched_gt_char:
            ...
            tw   = bbox[2] - bbox[0]
            # 予測文字 + conf
            pred_label = f"{pred_char} {pred_prob*100:.0f}"
            draw.text((x1, y1 - th - 5), pred_label,
...
if __name__ == "__main__":
    ...
    yolox      = YOLOXWrapper()
    model_conv = ConvNeXtSubCenterPredictor(
    ...
    for target_path in tqdm(image_paths, desc="Processing"):
        ...
        print("-"*60)
        print(f" {'ConvNeXtV2+SubCenter':<23}| "
              f"{scores['correct']:<7} | "
              f"{scores['recall']:.2f}%  | "
